...
def _estimate_stable_z_from_obj(obj_path: str, default: float = 0.12) -> float:
    ...
    if not z_values:
        return default
    # bin_b04-style meshes rest flush at z=0 (geometric min ~0), so -min(z) is
    # degenerately 0 -> fall back to `default`. toteweg-style meshes are centered
    # on the origin, where -min(z) is a real offset that must not be clamped up.
    z_min = min(z_values)
    if abs(z_min) < 1e-6:
        return default
    return -z_min
