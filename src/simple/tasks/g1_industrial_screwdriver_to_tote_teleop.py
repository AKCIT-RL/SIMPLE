...
@TaskRegistry.register("g1_industrial_screwdriver_to_tote_teleop")
class G1IndustrialScrewdriverToToteTeleop(Task):
    uid: str = "g1_industrial_screwdriver_to_tote_teleop"
    label: str = "G1 TELEOP Industrial Screwdriver to Tote to Workbench"
    description: str = (
        "An industrial task where the G1 robot must pick up a screwdriver from the assembly table, "
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language=LanguageDRCfg(
            instructions=[
                "pick up the screwdriver from the assembly table, put it in the tote, and move the tote to the dispatch workbench.",
            ]
        ),
        target=TargetDRCfg(asset_id="graspnet1b:19"),  # "blue screwdriver"
        container=TargetDRCfg(asset_id="totes:bin_b04"),
        distractors=DistractorDRCfg(
            res_id="graspnet1b",
            number_of_distractors=4,
            allow_duplicates=False,
            # Randomly sample 4 distinct distractors per episode from the
            # industrial-parts subset (8 power drill, 20 red screwdriver,
            # 26/29/30/31 toy workpieces, 27 metal screw, 28 yellow propeller,
            # 32 padlock, 70 tape), instead of the full graspnet1b pool. `exclude`
            # (not `include`) is what makes this a random draw: `include` would
            # force-load every listed id every episode instead of sampling.
            exclude=[
                str(i) for i in range(75)
                if i not in {8, 19, 20, 26, 27, 28, 29, 30, 31, 32, 70}
            ],
        ),
    ...
    def __init__(
        ...
        target_object: str | Object = "graspnet1b:19",
        ...
        **kwargs,
        ...
        if target_object is not None:
            ...
            if distractor_cfg is not None and isinstance(
                ...
                if distractor_cfg.include is not None and target_id in distractor_cfg.include:
                    distractor_cfg.include.remove(target_id)
    ...
    def check_object_in_container(self, *args, **kwargs) -> bool:
        """
        Check if the screwdriver is successfully placed in the tote.
        """
