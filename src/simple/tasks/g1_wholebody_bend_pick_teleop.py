...
class G1WholebodyBendPickTaskTeleop(Task):
    uid: str = "g1_wholebody_bend_pick_teleop"
    label: str = "G1 SONIC Bend Pick Teleop Task"
    description: str = (
        "A task where the G1 robot must bend and pick up an object."
    )
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.6, -0.0, 0.0], high=[-0.66, 0.0, 0.0]),
