...
@TaskRegistry.register("g1_industrial_tote_to_rack_teleop")
class G1IndustrialToteToRackTeleop(Task):
    uid: str = "g1_industrial_tote_to_rack_teleop"
    label: str = "G1 TELEOP Industrial Tote to Rack"
    description: str = (
        "An industrial task where the G1 robot must pick up a tote containing organized screws "
        "and place it onto an industrial rack/shelf."
    )
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language=LanguageDRCfg(
            instructions=[
                "pick up the tote with the screws and place it on the industrial rack.",
            ]
        ),
        target=TargetDRCfg(asset_id="graspnet1b:27"),  # "metal screw" inside the tote
        ...
        spatial=SpatialDRCfg(
            ...
            container_region=Box(low=[-0.10, 0.15], high=[-0.12, 0.25]),
        ...
        scene=TabletopSceneDRCfg(
            ...
            table2_height=Box(low=1.1, high=1.1), 
    ...
    def check_container_on_rack(self, *args, **kwargs) -> bool:
        """
        Check if the tote is on table2 (the industrial rack).
        """
    ...
    def compute_reward(self, info: dict[str, Any], *args, **kwargs) -> float:
        is_container_on_rack = self.check_container_on_rack(*args, **kwargs)

        if is_container_on_rack:
            self.reward = 1.0
        else:
            self.reward = 0.0
        return self.reward
